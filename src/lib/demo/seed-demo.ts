...
// (/api/cron/reset-demo) so the sandbox always returns to one known-good state
// and a prospect poking around can't permanently break the demo.
//
// Relative imports (not "@/…") so it also runs under tsx in the seed script.

export const DEMO_SLUG = "spice-garden-demo";

async function nextOrderSeq(prisma: PrismaClient, restaurantId: string): Promise<number> {
...
export async function seedDemoRestaurant(prisma: PrismaClient): Promise<{ orders: number; items: number }> {
  // Reset any prior demo data (cascades to all children: menu, tables, orders,
  // staff accounts, etc.). The platform super-admin is a separate account with
  // no restaurant, so it is untouched.
  ...
  // Attendance: one closed shift + one open, so the board has data.
  ...
  const categories = await Promise.all(
    [
      { name: "Starters", icon: "🍢", station: "KITCHEN" as const },
      { name: "Main Course", icon: "🍛", station: "KITCHEN" as const },
      { name: "Breads", icon: "🫓", station: "KITCHEN" as const },
      { name: "Beverages", icon: "🥤", station: "BAR" as const },
    ].map((c, i) =>
  ...
  const specs: Spec[] = [
    ...
    { tableIdx: 5, lines: [[3, 2], [7, 4], [9, 3]], status: "COMPLETED", paid: true, name: "Anita" },
  ];

  for (const spec of specs) {
    const lines = spec.lines.map(([idx, qty]) => ({ item: items[idx], qty }));
  ...
  return { orders: specs.length, items: items.length };
}
