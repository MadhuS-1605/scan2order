      ...
      {rooms.length === 0 ? (
        <Card>
          <p className="text-sm text-ink/55">
            {t(d, "rooms.emptyBefore")}{" "}
            <span className="font-medium text-ink/70">{t(d, "rooms.tablesAndQr")}</span>
            {t(d, "rooms.emptyAfter")}
          </p>
        </Card>
      ) : (
              ...
              <Card key={room.id} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-ink">{t(d, "rooms.room")} {room.label}</p>
                      <p className="text-xs text-ink/45">
                        {open.length
                          ? `${open.length} ${open.length > 1 ? t(d, "rooms.openCharges") : t(d, "rooms.openCharge")}`
                          : t(d, "rooms.noOpenCharges")}
                      </p>
                    </div>
                  </div>
                  ...
                  <>
                    <ul className="space-y-1 border-t border-sand-100 pt-2 text-sm">
                      {open.map((o) => (
                        <li key={o.id} className="flex justify-between text-ink/70">
                          <span>
                            #{o.orderNumber} · {o.items.length}{" "}
                            {o.items.length > 1 ? t(d, "rooms.items") : t(d, "rooms.item")}
                          </span>
                    ...
                    <form action={settleRoomAction}>
                      ...
                      >
                        ...
                        {t(d, "rooms.checkoutSettle")} {formatMoney(folio, cur)}
                      </button>
                    </form>
                  </>
                )}
