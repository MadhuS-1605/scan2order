...
import { setStockAction, restockAction, bulkEnableTrackingAction } from "@/lib/inventory/actions";
...
export function InventoryManager({ items }: { items: Item[] }) {
  ...
  const untracked = items.filter((it) => !it.trackStock).length;

  return (
    <div className="space-y-3">
      {untracked > 1 && (
        <form action={bulkEnableTrackingAction} className="flex justify-end">
          <Button size="sm" variant="secondary" type="submit">
            Enable tracking for all {untracked} untracked items
          </Button>
        </form>
      )}
      <Card className="p-0">
